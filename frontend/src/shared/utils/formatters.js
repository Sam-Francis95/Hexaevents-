...
/**
 * Tone to color the seats label/badge with, derived from the same "left"
 * math as seatsLabel above rather than duplicating it (Part H.5). Only ever
 * reflects real seat counts — no fabricated urgency.
 */
export function seatsUrgencyTone(capacity, registeredCount) {
  const left = Math.max(capacity - registeredCount, 0);
